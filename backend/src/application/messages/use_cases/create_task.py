...
from src.application.messages.commands import CreateTaskCommand
...
class CreateTask:
	...
	async def execute(self, command: CreateTaskCommand) -> UUID:
		section = await self.section_repo.get_by_id(command.section_id)

		section.ensure_allowed_message_type(MessageType.TASK)
