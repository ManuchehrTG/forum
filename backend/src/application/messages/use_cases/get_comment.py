...
from src.application.messages.queries import GetCommentQuery
...
class GetComment:
	...
	async def execute(self, query: GetCommentQuery) -> MessageDTO:
		message = await self.message_repo.get_by_id(query.comment_id)

		message.ensure_is_comment()
